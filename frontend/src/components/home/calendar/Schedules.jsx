...
const Schedules = ({ index }) => {
  const { schedules, selectSchedule, deleteSchedule } = useCalendarStore(
    (state) => {
      return {
  ...
  const openScheduleDetailView = useCallback(
    (id) => {
  ...
  const deleteScheduleHandle = useCallback(
    (schedule) => {
      deleteSchedule({
        ...schedule,
        index,
  ...
  const memorizedList = useMemo(() => {
    return schedules.map((schedule) => (
      <li
        ...
        key={schedule.id}
      >
        <div className="text-elipsis">{schedule.title}</div>
  ...
  return <ul className="schedules">{memorizedList}</ul>;
};
